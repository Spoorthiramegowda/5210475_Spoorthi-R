...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    int** seqList=calloc(n, sizeof(int*));
    ...
    for(int i=0;i<queries_rows;i++) {
        ...
        if(type==1) {
            if(seqSizes[seqIndex]==seqCapacity[seqIndex]){
                seqCapacity[seqIndex]=(seqCapacity[seqIndex]==0) ? 2 : seqCapacity[seqIndex]* 2;
                seqList[seqIndex]=realloc(seqList[seqIndex],seqCapacity[seqIndex]*sizeof(int));
            }
